...
예시:
- <example>
  상황: 사용자가 Next.js 애플리케이션의 초기 레이아웃 구조를 설정해야 함
  사용자: "프로젝트의 기본 레이아웃 구조를 설계해주세요"
  어시스턴트: "Next.js 앱 구조 설계 전문가를 사용하여 최적의 구조를 설계하겠습니다"
  <commentary>
  사용자가 레이아웃 아키텍처 설계를 필요로 하므로, nextjs-app-developer 에이전트를 사용하여 최적의 구조를 생성합니다.
  </commentary>
</example>
- <example>
  상황: 사용자가 올바른 라우팅을 포함한 페이지 구조를 생성하려고 함
  사용자: "대시보드, 프로필, 설정 페이지를 포함한 앱 구조를 만들어주세요"
  어시스턴트: "nextjs-app-developer 에이전트를 활용하여 페이지 구조와 라우팅을 설계하겠습니다"
  <commentary>
  사용자가 라우팅이 포함된 여러 페이지를 필요로 하므로, nextjs-app-developer 에이전트가 완벽합니다.
  </commentary>
</example>
- <example>
  상황: 사용자가 중첩된 레이아웃이 필요한 관리자 섹션을 구현해야 함
  사용자: "중첩된 레이아웃이 필요한 관리자 섹션을 구성해주세요"
  어시스턴트: "Next.js 앱 구조 전문가를 통해 중첩 레이아웃 구조를 구현하겠습니다"
  <commentary>
  중첩 레이아웃은 전문적인 Next.js 지식이 필요하므로, nextjs-app-developer 에이전트를 사용합니다.
  </commentary>\n</example>
model: sonnet
color: blue
---
